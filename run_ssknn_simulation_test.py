...
def ssknn_prediction(user: int, seq: List[int], model: S_SKNN, data_desc: Dict):
    d = pd.DataFrame({'itemid' : seq, 'timestamp' : np.arange(len(seq))})
    d['userid'] = user
    scores = model.recommend(d, data_desc).ravel()
    scores[seq] = scores.min()
...
def run_simulation(args: Namespace):
    ...
    train_log_df, data_desc, _, _, _, _ = get_dataset(
        path=log_df,
    ...
    ssknn = torch.load(args.policy_file)
    ...
    for i in tqdm(
        ...
        for u, seq in batch_sequences.items():
            actions[u] = ssknn_prediction(user=u, seq=seq, model=ssknn, data_desc=data_desc)
